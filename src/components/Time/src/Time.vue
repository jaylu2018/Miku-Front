...
export default defineComponent({
  ...
  setup(props) {
    ...
    function getRelativeTime(timeStamp: number) {
      ...
      let resStr = ''
      let dirStr = isBefore
        ? t('component.time.before')
        : t('component.time.after')
      ...
      if (diff < ONE_SECONDS) {
        ...
        resStr =
          parseInt(diff / ONE_SECONDS) + t('component.time.seconds') + dirStr
        // More than 59 seconds, less than or equal to 59 minutes and 59 seconds
      } else if (diff >= ONE_MINUTES && diff < ONE_HOUR) {
        resStr =
          Math.floor(diff / ONE_MINUTES) + t('component.time.minutes') + dirStr
        ...
        resStr =
          Math.floor(diff / ONE_HOUR) + t('component.time.hours') + dirStr
