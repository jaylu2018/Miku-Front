<template>
  <div :class="`${prefixCls}-toolbar`" class="flex items-center px-2 py-1">
    <template v-for="item in toolbarItemList" :key="item.type">
      <Tooltip
        placement="bottom"
        v-bind="item.disabled ? { visible: false } : {}"
      >
        ...
        >
          <Icon
            :icon="item.icon"
            :class="
              item.disabled ? 'cursor-not-allowed disabeld' : 'cursor-pointer'
            "
          />
        </span>
      </Tooltip>
...
export default defineComponent({
  ...
  setup(_, { emit }) {
    ...
    function onHistoryChange({ data: { undoAble, redoAble } }) {
      ...
      const undoIndex = itemsList.findIndex(
        (item) => item.type === ToolbarTypeEnum.UNDO
      )
      const redoIndex = itemsList.findIndex(
        (item) => item.type === ToolbarTypeEnum.REDO
      )
      if (undoIndex !== -1) {
